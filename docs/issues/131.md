# Issue #131: Add ability to delete individual quizzes from history

**GitHub:** https://github.com/vitorsilva/saberloop/issues/131
**Status:** Implementation Complete - Ready for Review
**Created:** 2026-01-16
**Labels:** enhancement, ux
**Branch:** `feature/issue-131-delete-individual-quiz`
...
- [x] Delete button visible on each quiz card in history (TopicsView)
- [x] Clicking delete shows confirmation modal with quiz topic name
- [x] Confirming deletion removes quiz from IndexedDB
- [x] Quiz card is removed from the list after deletion
- [x] Success toast shown after deletion
- [x] Sample quizzes can also be deleted individually
- [x] Delete button works on mobile (touch-friendly size)
- [x] i18n: All strings translated (PT, EN at minimum)
...
- [x] Delete button appears on quiz cards
- [x] Delete button is clickable (doesn't trigger replay)
- [x] Confirmation modal shows quiz topic
- [x] Cancel closes modal without deleting
- [x] Confirm deletes quiz and updates list
- [x] Toast confirms deletion
- [x] Works on mobile/touch devices
- [x] Works for sample quizzes
...
|------------|----------|-------------|
| Before | `131-before.png` | History view without delete button |
| After | `131-after.png` | History view with delete button visible |
| Modal | `131-modal.png` | Confirmation modal when deleting |
...
1. **Database Layer** (`src/core/db.js:136-139`)
   - Added `deleteSession(id)` function using idb's `db.delete()` method

2. **Service Layer** (`src/services/quiz-service.js:101-113`)
   - Added `deleteQuiz(id)` wrapper with existence validation

3. **i18n** (all 9 locale files)
   - Added `history.deleteQuiz`, `deleteQuizConfirm`, `deleteQuizDescription`, `quizDeleted` strings
   - Languages: English, Portuguese, German, Spanish, French, Italian, Dutch, Norwegian, Russian

4. **UI Layer** (`src/views/TopicsView.js`)
   - Added delete button (red trash icon) to each quiz card
   - Added click handler with `e.stopPropagation()` to prevent replay
   - Added `handleDeleteQuiz()` method that shows modal and deletes on confirm
   - Added `showSuccessToast()` method (following existing SettingsView pattern)

5. **Modal Component** (`src/components/DeleteQuizModal.js`)
   - Created new modal following DeleteDataModal pattern
   - Promise-based API returning boolean
   - Backdrop click, escape key, and cancel button to dismiss
   - Includes `data-testid` attributes for E2E testing

### Tests Added

1. **Unit Tests** (`src/core/db.test.js`)
   - `deleteSession` describe block with 4 tests
   - Tests deletion, non-existent ID handling, selective deletion, sample quiz deletion

2. **E2E Tests** (`tests/e2e/quiz-delete.spec.js`)
   - 7 tests covering UI, modal behavior, and actual deletion
   - Verifies toast notification and quiz list update

### Design Decisions

- Used local `showSuccessToast()` method rather than creating a shared utility (follows existing SettingsView pattern)
- Delete button always visible (not just on hover) for better mobile UX
- Topic name stored in `data-topic` attribute with HTML entity escaping for quotes
...
```
feature/issue-131-delete-individual-quiz
```
...
| 7 | `docs(issues): update plan for issue 131` |
