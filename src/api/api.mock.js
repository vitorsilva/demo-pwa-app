...
export async function generateQuestions(
  ...
  const delay =
    (typeof window !== 'undefined' && /** @type {*} */ (window).MOCK_API_DELAY_MS) || 1000;
  ...
  logger.debug('Mock API generating questions', {
    topic,
    gradeLevel,
    questionCount,
    ...
    mockQuestions.push({
      ...
      question:
        ...
        (i >= questionTemplates.length
          ? ` (Part ${Math.floor(i / questionTemplates.length) + 1})`
          : ''),
    });
  }
...
export async function generateExplanation(_question, userAnswer, correctAnswer, settings = {}) {
  const {
    gradeLevel: _gradeLevel = 'middle school',
    apiKey: _apiKey,
  ...
  if (typeof window !== 'undefined' && /** @type {*} */ (window).__MOCK_EXPLANATION_ERROR__) {
    ...
    const error = /** @type {Error & {code?: string}} */ (
      new Error(errorConfig.message || 'Mock error')
    );
    if (errorConfig.code) {
